...
<?php  
  $sql="select * from `$tbpengajar` order by `nama_pengajar` asc";
  $jum=getJum($conn,$sql);
  $no=0;
		if($jum > 0){
...
echo"<img src='$YPATH/$gambar' width='40' height='40' /></a></div>";
				echo"</td>
				<td valign='top'><strong>$id_pengajar | $nama_pengajar ($jk)</strong>
...
echo"<img src='$YPATH/$gambar' width='40' height='40' /></a></div>";
		...
		else{echo"<tr><td colspan='7'><blink>Maaf, Data pengajar belum tersedia...</blink></td></tr>";}
