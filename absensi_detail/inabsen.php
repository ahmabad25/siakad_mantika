...
<td colspan="2">	<input name="Simpan" type="submit" id="Simpan" value="Simpan" />
        ...
        <a href="?mnu=inabsen&id=<?php echo $id_peserta; ?>"><input name="Batal" type="button" id="Batal" value="Batal" /></a>
</td></tr>
</table>
</form>
<!-----ACCORDION----->
...
<table width="100%" border="0" class="table table-striped table-bordered table-hover">
  <tr bgcolor="#036">
    ...
    <th width="25%"><center>Catatan</center></th>
...
echo"<tr bgcolor='$color'>
				...
				<td align='center'>
<a href='?mnu=inabsen&id=$id_absensi&pro=ubah&kode=$id'><img src='ypathicon/u.png' alt='ubah'></a>
<a href='?mnu=inabsen&id=$id_absensi&pro=hapus&kode=$id'><img src='ypathicon/h.png' alt='hapus' 
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=inabsen'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=inabsen'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=inabsen'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
		if($simpan) {echo "<script>alert('Data $id_siswa berhasil disimpan !');document.location.href='?mnu=inabsen&id=$id_absensi';</script>";}
		else{echo"<script>alert('Data $id_siswa gagal disimpan...');document.location.href='?mnu=inabsen&id=$id_absensi';</script>";}
	}
	else{
...
$ubah=process($conn,$sql);
	if($ubah) {echo "<script>alert('Data $id_siswa berhasil diubah !');document.location.href='?mnu=inabsen&id=$id_absensi';</script>";}
	else{echo"<script>alert('Data $id_siswa gagal diubah...');document.location.href='?mnu=inabsen&id=$id_absensi';</script>";}
	}//else simpan
}
?>
...
if($hapus) {echo "<script>alert('Data absensi_detail $id berhasil dihapus !');document.location.href='?mnu=inabsen&id=$id_absensi';</script>";}
else{echo"<script>alert('Data absensi_detail $id gagal dihapus...');document.location.href='?mnu=inabsen&id=$id_absensi';</script>";}
}
?>
