...
?>
<a class="btn btn-default btn=lg" href="?mnu=absensi&id=<?php echo $id_kelas;?>"><img src='ypathicon/10.png'> Back to Attendance Date's Page</a>
<br>
<!-----ACCORDION----->
<hr id="myhr">
...
<tr>
<td width="199" height="24"><label for="tanggal_pertemuan">&emsp;Attendance Date</label></td>
<td width="12" valign="top">:</td>
<td width="440" colspan="3"><b><?php echo $tanggal_pertemuan=date("d M Y");?></b></td>
</tr>

<tr>
<td width="199" height="24"><label for="id_siswa">&emsp;Student</label></td>
<td width="12" valign="top">:</td>
...
<td colspan="2">
    ...
    <a href="?mnu=absensi_detail&id1=<?php echo"$id_kelas&id2=$id_absensi";?>"><input name="Batal" type="button" id="Batal" value="Cancel" /></a>
</td></tr>
</table>
</form>
<hr id="myhr">
...
<img src='$YPATH/$gambar' width='40' height='40' /></a></div>";
				...
				<td>$catatan</td>
				<td align='center'>
<a href='?mnu=absensi_detail&id1=$id_kelas&id2=$id_absensi&pro=ubah&kode=$id'><img src='ypathicon/12.png' title='Change'></a>
<a href='?mnu=absensi_detail&id1=$id_kelas&id2=$id_absensi&pro=hapus&kode=$id'><img src='ypathicon/h.png' title='Delete' 
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=absensi_detail'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=absensi_detail'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=absensi_detail'>Next »</a></span>";
	}
...
if($pro=="simpan"){
	$sql="select * from `$tbabsensidetail` where id_siswa='$id_siswa' and id_absensi='$id_absensi'";
  	$jum=getJum($conn,$sql);
		if($jum > 0){
		echo"<script>alert('ATTENDANCE FOR THE SELECTED STUDENT HAS ALREADY BEEN INPUT BEFORE!');document.location.href='?mnu=absensi_detail&id1=$id_kelas&id2=$id_absensi';</script>";	
		}
	else{	
...
$simpan=process($conn,$sql);
		if($simpan) {echo "<script>alert('SAVE SUCCESS');document.location.href='?mnu=absensi_detail&id1=$id_kelas&id2=$id_absensi';</script>";}
		else{echo"<script>alert('SAVE FAILED');document.location.href='?mnu=absensi_detail&id1=$id_kelas&id2=$id_absensi';</script>";}
	}
}
	else{
...
$ubah=process($conn,$sql);
	if($ubah) {echo "<script>alert('UPDATE SUCCESS');document.location.href='?mnu=absensi_detail&id1=$id_kelas&id2=$id_absensi';</script>";}
	else{echo"<script>alert('UPDATE FAILED');document.location.href='?mnu=absensi_detail&id1=$id_kelas&id2=$id_absensi';</script>";}
	}//else simpan
}
?>
...
if($hapus) {echo "<script>alert('DELETE SUCCESS');document.location.href='?mnu=absensi_detail&id1=$id_kelas&id2=$id_absensi';</script>";}
else{echo"<script>alert('DELETE FAILED');document.location.href='?mnu=absensi_detail&id1=$id_kelas&id2=$id_absensi';</script>";}
}
?>
