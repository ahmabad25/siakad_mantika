...
$ypath="../ypathfile";
$tanggal_pertemuan=WKT(date("d m Y"));
?>
...
<h3 align="center"><b>Attendance Report of <?php echo"$level ($ccal $pengajar's Class)";?><br>Program : <?php echo $program;?></b></h3>
<hr>
...
<?php  
	$sx="SELECT * FROM tb_absensi WHERE id_kelas='$id_kelas' order by tanggal_pertemuan asc";
	$no=0;
	$jx=getJum($conn,$sx);
		if($jx<1){
	...
	$ax=getData($conn,$sx);
		foreach($ax as $dx) {
			$id_absensi=$dx["id_absensi"];
			$tanggal_pertemuan=WKT($dx["tanggal_pertemuan"]);
			$hari=date('l', strtotime(($dx["tanggal_pertemuan"])));
			$no++;
?>
<h4>&emsp;<?php echo"$no)&emsp;$hari, $tanggal_pertemuan";?> : </h4>
<table width="100%" border="1" cellpadding="5">
  <tr>
    <th width="10%"><center>No.</th>
    <th width="50%"><center>Student</th>
    <th width="10%"><center>Status</th>
    <th width="30%"><center>Details</th>
  </tr>
<?php  
  $sy="SELECT id, nama_siswa, (tb_absensi_detail.status) as status, catatan from `$tbabsensidetail`, `$tbsiswa` where tb_absensi_detail.id_siswa=tb_siswa.id_siswa and id_absensi='$id_absensi' order by tb_siswa.nama_siswa asc";
  $jy=getJum($conn,$sy);
  $noin=0;
		if($jy>0){
			$ay=getData($conn,$sy);
...
echo"<tr bgcolor='#cccccc'>
		...
		else{echo"<tr><td colspan='4'><blink>Maaf, Data absensi belum tersedia...</blink></td></tr>";}
