...
<script type="text/javascript"> 
function PRINT(){ 
win=window.open('absensi/print.php','win','width=1000, height=400, menubar=0, scrollbars=1, resizable=0, location=0, toolbar=0, id_periode=0'); } 
</script>
...
function showPengajar(str){ 
xmlHttp=GetXmlHttpObject()
if (xmlHttp==null){
 alert ("Browser tidak support HTTP Request")
 return
 } 
var url="getpengajar.php"
url=url+"?q="+str
...
<td colspan="2">	<input name="Simpan" type="submit" id="Simpan" value="Save" />
        ...
        <a href="?mnu=pabsensi&id=<?php echo $id_kelas?>"><input name="Batal" type="button" id="Batal" value="Cancel" /></a>
</td></td></tr>
</table>
</form>
<hr id="myhr">
...
<div id="accordion">
<?php  
  $sqlp="SELECT tb_kelas.id_kelas, tb_program.nama_program, tb_program.level FROM tb_kelas JOIN tb_program ON tb_kelas.id_program = tb_program.id_program WHERE id_kelas = '$id_kelas'";
  $jump=getJum($conn,$sqlp);
		if($jump <1){
			echo"<h1>No data available...</h1>";
			}								
	$dp=getField($conn,$sqlp);
			$program=$dp["nama_program"];
			$level=$dp["level"];
?>    
...
<?php  
  ...
  $jum=getJum($conn,$sql);
	...
	echo"<tr bgcolor='$color'>
				...
				<td align='center'><b>$tanggal_pertemuan<b></td>
				<td>Input on <i>$tanggal_input</i> at <i>$waktu_input WIB</i></td>
				<td align='center'>
<a href='?mnu=pabsensi_detail&id1=$id_kelas&id2=$id_absensi'><img src='ypathicon/add-presention.png' title='Input Student Attendance'></a>	
<a href='?mnu=pabsensi&id=$id_kelas&pro=ubah&kode=$id_absensi'><img src='ypathicon/12.png' title='Change'></a>
<a href='?mnu=pabsensi&id=$id_kelas&pro=hapus&kode=$id_absensi'><img src='ypathicon/h.png' title='Delete' 
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=pabsensi'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=pabsensi'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=pabsensi'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
		if($simpan) {echo "<script>alert('SAVE SUCCESS');document.location.href='?mnu=pabsensi&id=$id_kelas';</script>";}
		else{echo"<script>alert('SAVE FAILED');document.location.href='?mnu=pabsensi&id=$id_kelas';</script>";}
	}
}
	else{
...
$ubah=process($conn,$sql);
	if($ubah) {echo "<script>alert('UPDATE SUCCESS');document.location.href='?mnu=pabsensi&id=$id_kelas';</script>";}
	else{echo"<script>alert('UPDATE FAILED');document.location.href='?mnu=pabsensi&id=$id_kelas';</script>";}
	}//else simpan
}
?>
...
if($hapus) {echo "<script>alert('DELETE SUCCESS');document.location.href='?mnu=pabsensi&id=$id_kelas';</script>";}
else{echo"<script>alert('DELETE FAILED');document.location.href='?mnu=pabsensi&id=$id_kelas';</script>";}
}
?>
