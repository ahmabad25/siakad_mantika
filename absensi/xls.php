...
if ($conn->connect_error) {
  trigger_error('Database connection failed: '  . $conn->connect_error, E_USER_ERROR);
    ...
    $buffer = "id_absensi".$separator ."tanggal".$separator ."jam".$separator ."id_kelas".$separator ."id_pengajar".$separator ."id_periode".$separator; 
    $buffer .= $newline; 
    
  $sql="select `id_absensi`,`tanggal`,`jam`,`id_kelas`,`id_pengajar`,`id_periode` from `$tbabsensi` order by `id_absensi` desc";
  $jum=getJum($conn,$sql);	
  if($jum>0){						
